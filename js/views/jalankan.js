...
export async function renderJalankan(root) {
  ...
  runBtn.addEventListener('click', async () => {
    ...
    statusMsg.textContent = result.usage
      ...
      : 'Selesai.';
    document.getElementById('ai-model-label').textContent = result.model || config.model;
    output.textContent = result.text;
    lastAnswer = { prompt: rendered.text, answer: result.text, model: result.model || config.model };
